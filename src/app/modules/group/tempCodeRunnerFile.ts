...
const createGroupIntoDB = async (payload: TGroup) => {
  ...
  console.log(payload)
  const members = await User.find({ _id: { $in: payload.members } });
  console.log(members)
  // return err if author or members not found
  if (!author || members?.length !== payload?.members?.length) {
    throw new AppError(httpStatus.NOT_FOUND, "Author or Member Not Found");
  }

  // create an array off members with current user as admin 
  const membersWithRole = members?.map((member) => ({
    userId: member?._id,
    role: member?._id?.equals(author?._id) ? "admin" : "member",
    acceptInvitation: member?._id?.equals(author?._id),
  }));
...
//   return result;
// };
// const updateTaskIntoDB = async (id: string, payload: Partial<TGroup>) => {
